...
const features: Array<Feature> = [
  ...
  {
    name: 'Risk & limits',
    summary: 'Cap daily risk and positions with built-in guardrails.',
    description:
    ...
    icon: function RiskShieldIcon() {
      return (
        <>
          <path d="M18 4 28 8v6c0 7.4-4.6 12.5-10 13.9C12.6 26.5 8 21.4 8 14V8l10-4Z" fill="#fff" opacity=".25" />
          <path d="M12.5 18.5 16 22l8-8" stroke="#fff" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
        </>
      );
    },
  },
  {
    name: 'Bot rules',
    summary: 'Define how orders behave—SL/TP, breakeven, trailing.',
    description:
...
function Feature({
  ...
  return (
    <div className={clsx(className, !isActive && 'opacity-75 hover:opacity-100')} {...props}>
      <div className={clsx('w-9 rounded-lg', isActive ? 'bg-[#3f4bf2]' : 'bg-gray-400/80')}>
        <svg aria-hidden="true" className="h-9 w-9" fill="none">
          <feature.icon />
        </svg>
      </div>
      <h3 className={clsx('mt-6 text-sm font-medium', isActive ? 'text-[#3f4bf2]' : 'text-gray-400/80')}>{feature.name}</h3>
    ...
    <TabGroup className="hidden lg:mt-12 lg:block">
      {({ selectedIndex }) => (
        <>
          ...
          <TabPanels className="relative mt-12 overflow-hidden rounded-4xl bg-[#3f4bf2]/50 px-14 py-14 xl:px-16">
            <div className="-mx-5 flex">
              {features.map((feature, featureIndex) => (
                <TabPanel
                  static
                  key={feature.summary}
                  className={clsx('px-5 transition duration-500 ease-in-out data-selected:not-data-focus:outline-hidden', featureIndex !== selectedIndex && 'opacity-60')}
...
export function SecondaryFeatures() {
  return (
    <section id="features" aria-label="Features for simplifying everyday business tasks" className="pt-16 pb-8 sm:pt-24 sm:pb-12 lg:pb-16">
      <Container>
        <div className="mx-auto max-w-2xl md:text-center">
          <h2 className="font-display text-3xl tracking-tight text-white sm:text-4xl">Simplify everyday trading ops.</h2>
          <p className="mt-4 text-lg tracking-tight text-gray-400/80">You shouldn’t babysit charts. The dashboard keeps your bots compliant, fast, and under control.</p>
        </div>
        <FeaturesMobile />
        <FeaturesDesktop />
      </Container>
